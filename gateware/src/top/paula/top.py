...
channel 0. MIDI NOTE_ON events on channel 10 control the transport:

- note 48: toggle record start/stop
- note 47: trigger one-shot playback from sample start
"""
...
class PaulaRecorderCore(wiring.Component):
    ...
    RECORD_SECONDS = 2
    RECORD_FS = 48000
    MAX_SAMPLES = RECORD_SECONDS * RECORD_FS
    ...
    def elaborate(self, platform):
        ...
        m.submodules.sample_mem = sample_mem = Memory(
            shape=signed(8), depth=self.MAX_SAMPLES, init=[]
        )
        wr = sample_mem.write_port()
        rd = sample_mem.read_port()

        recording = Signal(init=0)
        valid_length = Signal(range(self.MAX_SAMPLES + 1), init=0)
        write_addr = Signal(range(self.MAX_SAMPLES + 1), init=0)

        play_state = Signal(self.PlayState, init=self.PlayState.IDLE)
        play_addr = Signal(range(self.MAX_SAMPLES + 1), init=0)
        play_count = Signal(range(self.MAX_SAMPLES + 1), init=0)
        ...
        m.d.comb += [
            ...
            do_read.eq(do_prefetch | (do_play_emit & ~play_last)),
            rd.en.eq(do_read),
        ...
        with m.If(do_write):
            ...
            with m.If((write_addr + 1) >= self.MAX_SAMPLES):
                m.d.sync += recording.eq(0)
        ...
        with m.If(do_play_emit):
            m.d.sync += play_count.eq(play_count + 1)
            with m.If(play_last):
                m.d.sync += [
                    play_state.eq(self.PlayState.IDLE),
                    play_addr.eq(0),
            ...
            with m.Else():
                m.d.sync += play_addr.eq(play_addr + 1)
